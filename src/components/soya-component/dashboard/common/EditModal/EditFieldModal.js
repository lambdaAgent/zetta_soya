...
class EditFieldModal extends React.Component {
  componentWillMount() {
    ...
    this.props.context.store.dispatch(this.actions.setValues(this.props.namespace, [{
      fieldName: this.props.modalId, value: this.props.data.value
    }]));
  }
  ...
  save() {
    this.form.submit((result) => {
      ...
      this.props.handleSave(
        this.props.modalId,
        result.values[this.props.modalId],
  ...
  render() {
    ...
    return <div className={style.simpleModal} style={{top: `${topOffset}px`, left: `${leftOffset}px`}}>
      <div className={style.modalBody}>
        ...
        <TextBox name={this.props.modalId} placeholder={this.props.data.value} form={this.form}
                 changeValidators={[required]} context={this.props.context} />
        {this.props.data.reason ?
          <div>
            <h4>Reason</h4>
            <TextArea name={`${this.props.modalId}-reason`} form={this.form} minHeight={100}
                      changeValidators={[required]} context={this.props.context} />
          </div>
        : null}
      </div>
...
export default createModal(EditFieldModal);